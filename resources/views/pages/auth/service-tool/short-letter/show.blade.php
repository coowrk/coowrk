<div wire:poll.2s>
	...
	<livewire:pages.auth.service-tool.short-letter.show.add-diary-entry-dialog short_letter="{{ $short_letter->id }}" />
	...
	<div class="mt-2.5 flex flex-wrap items-end justify-between gap-x-6 gap-y-4">
		<div class="flex flex-wrap gap-x-10 gap-y-4 py-1.5">
			<div class="text-base-/6 flex items-end gap-3 text-zinc-950 dark:text-zinc-100 sm:text-sm/6">
				<svg
					class="size-5 sm:size-4 shrink-0 fill-zinc-400 dark:fill-zinc-500"
					fill="currentColor"
					viewBox="0 0 20 20"
					xmlns="http://www.w3.org/2000/svg"
				>
				...
				<div class="flex flex-col">
					<span class="text-sm/8 text-zinc-500 dark:text-zinc-400">Erstellt</span>
					<span class="text-sm/4 font-medium">{{ $short_letter->created_at->diffForHumans() }}</span>
				</div>
			</div>
			...
			<div class="text-base-/6 flex items-end gap-3 text-zinc-950 dark:text-zinc-100 sm:text-sm/6">
				...
				<div class="flex flex-col">
					<span class="text-sm/8 text-zinc-500 dark:text-zinc-400">Erstellt von</span>
			...
			<div class="text-base-/6 flex items-end gap-3 text-zinc-950 dark:text-zinc-100 sm:text-sm/6">
				...
				<div class="flex flex-col">
					<span class="text-sm/8 text-zinc-500 dark:text-zinc-400">Zugehöriger Kunde</span>
					<span class="text-sm/4 font-medium">
		...
		<div class="flex h-fit gap-4">
			<x-button
				href="{{ route('service-tool.short-letter.pdf', $short_letter->id) }}"
				target="_blank"
				theme="blank"
				type="button"
			>
			...
			<x-button
				href="{{ route('service-tool.short-letter.edit', $short_letter->id) }}"
				type="button"
				wire:navigate
			>
				<svg
		...
		<div class="grid grid-cols-1 gap-4 sm:gap-10 md:grid-cols-12">
			...
			<div class="col-span-1 flex flex-col gap-y-12 md:col-span-7">
				<div>
					...
					<x-descriptionlist.base>
						...
						<x-descriptionlist.detail>
							<div class="flex">
								<span class="block w-14 text-zinc-500 dark:text-zinc-400">{{ $short_letter->salutation }}</span>
								{{ $short_letter->first_name }}
								{{ $short_letter->last_name }}
							</div>
						</x-descriptionlist.detail>
						<x-descriptionlist.term text="Straße" />
						<x-descriptionlist.detail text="{{ $short_letter->street }} {{ $short_letter->house_number }}" />
						<x-descriptionlist.term text="Stadt" />
						<x-descriptionlist.detail text="{{ $short_letter->postcode }} {{ $short_letter->city }}" />
						<x-descriptionlist.term text="Land" />
						<x-descriptionlist.detail text="{{ $short_letter->country }}" />
					</x-descriptionlist.base>
				</div>
				...
				<div>
					...
					<x-descriptionlist.base>
						<x-descriptionlist.term text="Betreff" />
						<x-descriptionlist.detail text="{{ $short_letter->reason }}" />
						<x-descriptionlist.term text="Optionen" />
						<x-descriptionlist.detail>
